...
class Search
  ...
  def self.search(term)
    ...
    Excon.post("#{QUEUE_HOST}/messages", query: {message: {topic: 'search', body: {term: term} }.to_json} )
...
get '/api/play/:yt_id' do
  Excon.post("#{QUEUE_HOST}/messages", query: {message: {topic: 'extract', body: {id: params[:yt_id], fulltitle: params[:name], img: params[:img]} }.to_json} )
end
...
get '/tracks/:yt_id/remove' do
  Excon.post("#{QUEUE_HOST}/messages", query: {message: {topic: 'remove', body: {id: params[:yt_id]} }.to_json} )
  'ok'
end
